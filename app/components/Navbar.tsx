...
export default function Navbar() {
  const links = [
    ...
    { name: "Dinner", href: "#dinner" } // <-- Added dinner target link
  ];
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white/60 backdrop-blur-md z-50 border-b border-slate-200/40 flex justify-center items-center py-4">
      <div className="flex items-center gap-4 sm:gap-8 text-xs font-bold tracking-widest uppercase text-slate-600">
        {links.map((link, i) => (
          <motion.a 
            key={i}
            href={link.href} 
